...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "property_db.sqlite")
...
def get_featured_properties():
    ...
    cursor.execute("""
        SELECT 
            ...
            price, total_area, baths, has_balcony,
            price_value
        FROM properties 
        ORDER BY RANDOM() 
        LIMIT 6
    ...
    return jsonify([{
        ...
        "location": p[2],
        "city": p[3],
        "state": p[4],  # neighborhood is used as state in frontend
        "price": p[5],
        "total_area": p[6],
        "bathrooms": p[7],
        "images": [{"url": "/assets/images/default-property.jpg"}],  # Default image
        "bedrooms": 2  # Default value as it's extracted from title in rec.py
    } for p in properties])
...
# Keep only one version of search_properties
...
def search_properties():
    try:
        ...
        # First try SQL search for exact matches
        ...
        # If SQL search returns no results, try recommender system
        if not sql_results:
            recommender_results, relaxed_search = recommender.search_properties_with_fallback(filters)
            return jsonify({
                "results": recommender_results,
                "relaxed_search": relaxed_search,
                "search_type": "recommender"
            })
