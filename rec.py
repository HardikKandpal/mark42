...
class PropertyRecommender:
    ...
    def get_recommendations(self, location, min_price, max_price, bedrooms, property_type, num_recommendations=5):
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            # Find similar properties based on input criteria
            query = """
            SELECT id, title, location, city, price, price_value, total_area, baths, has_balcony
            FROM properties
            WHERE (city LIKE ? OR location LIKE ? OR neighborhood LIKE ?)
            AND price_value BETWEEN ? AND ?
            AND title LIKE ?
            AND title LIKE ?
            LIMIT ?
            """
